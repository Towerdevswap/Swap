...
export function useGasPrice(chainIdOverride?: number): string {
  ...
  const { data } = useFeeData({
    chainId,
    enabled: chainId !== ChainId.ONLY && chainId !== ChainId.ONLY,
    watch: true,
  })
  if (chainId === ChainId.ONLY) {
...
export function toV2LiquidityToken([tokenA, tokenB]: [ERC20Token, ERC20Token]): ERC20Token {
  return new ERC20Token(tokenA.chainId, Pair.getAddress(tokenA, tokenB), 18, 'OnlySwap LPs', 'OnlySwap-LP')
}
