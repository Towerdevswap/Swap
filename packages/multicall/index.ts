...
export const multicallAddresses = {
  ...
  5820948: '0xCD3F8368f9d2489292208090aD26bb35a6562E2B',
}
...
export function createMulticall<TProvider extends Provider>(
  ...
  const multicall: MultiCall = async (abi: any[], calls: Call[], chainId = ChainId.ONLY) => {
  ...
  const multicallv2: MultiCallV2 = async ({ abi, calls, chainId = ChainId.ONLY, options, provider: _provider }) => {
  ...
  const multicallv3 = async ({ calls, chainId = ChainId.ONLY, allowFailure, overrides }: MulticallV3Params) => {
    const multi = getMulticallContract(chainId, provider({ chainId }))
    if (!multi) throw new Error(`Multicall Provider missing for ${chainId}`)
