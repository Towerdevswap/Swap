...
const supportedChainId = [ChainId.ONLY]
...
export function createFarmFetcher(multicallv2: MultiCallV2) {
  const fetchFarms = async (
    ...
    const masterChefAddress = isTestnet ? masterChefAddresses[ChainId.BSC] : masterChefAddresses[ChainId.ONLY]
  ...
  return {
    ...
    supportedChainId,
    isTestnet: (chainId: number) => ![ChainId.ONLY, ChainId.ETHEREUM].includes(chainId),
  }
}
